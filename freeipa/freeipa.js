...
exports.getUser = async function (server='127.0.0.1', base='cn=accounts,dc=example,dc=org', binduser='readonly', bindpass='password', uid='test', group='chat'){
    var client = new Client({url: 'ldaps://'+server});
    var bindid = 'uid='+binduser+',cn=users,'+base;
    return new Promise(function (resolve, reject) {
      client.bind(bindid,bindpass).then(function () {
        var grf = '(memberof=cn='+group+',cn=groups,'+base+')';
        var opts = { filter: '(&(employeeNumber='+ uid +')'+grf+')', scope: 'sub'};
